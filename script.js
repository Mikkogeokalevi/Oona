...
const nextLevelButton = { x: 0, y: 0, width: 250, height: 50 };
...
function handlePlayerHit() {
    ...
    if (player.lives <= 0) {
        ...
        obstacles = obstacles.filter(obs => {
            ...
            if (direction === 1) return obs.x < player.x - 50 || obs.x > player.x + 150;
            return obs.x > player.x + 50 || obs.x < player.x - 150;
        });
    }
}
...
function updateGame() {
    ...
    if (spawnCondition) {
        ...
        const x = direction === 1 ? canvas.width + 5 : -65;
        let newObstacle;
        switch (obstacleType) {
